...
LIVENESS_URL = "http://www.gstatic.com/generate_204"
...
async def replenish(pool: AsyncConnectionPool) -> None:
    ...
    results = await asyncio.gather(*(_validate(p, sem) for p in cand))
    good = [r for r in results if r]
    print(f"[replenish] активных {active}/{TARGET}, проверено {len(cand)}, живых +{len(good)}", flush=True)
    if good:
        async with pool.connection() as conn:
