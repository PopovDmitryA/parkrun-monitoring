...
SOURCES = [
    # Проверено 26.07.2026: каждый URL реально отдаёт ip:port (мёртвые 404 убраны —
    # они впустую жгли время harvest). Отсортировано по объёму отдачи.
    # НЕ добавлять mishakorzik/Free-Proxy/main/proxy.txt: 1М записей (86% всего пула
    # кандидатов), из них за всё время НИ ОДНОГО рабочего прокси — он лишь вытеснял
    # полезные адреса из окна скана CAND_CAP.
...
DELAY = float(os.getenv("PM_FREE_DELAY", "55"))           # задержка между атлетами на прокси
...
DELAY_FLOOR = float(os.getenv("PM_FREE_DELAY_FLOOR", "45"))  # ниже суточный тюнинг не опускает
