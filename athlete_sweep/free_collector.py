...
TCP_CONC = int(os.getenv("PM_FREE_TCP_CONC", "800"))        # префильтр дешёвый — можно много
VALIDATE_CONC = int(os.getenv("PM_FREE_VALIDATE_CONC", "50"))
VALIDATE_BATCH = int(os.getenv("PM_FREE_VALIDATE_BATCH", "4000"))  # parkrun-проверка TCP-живых
...
async def replenish(pool: AsyncConnectionPool) -> None:
    ...
    if not cand:
        return
    # Ступень 1 — дешёвый TCP-префильтр по ВСЕМ кандидатам (капа охвата нет).
