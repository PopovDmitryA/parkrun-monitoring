...
CAND_CAP = int(os.getenv("PM_FREE_CAND_CAP", "6000"))       # адресов на TCP-скан за цикл
...
async def replenish(pool: AsyncConnectionPool) -> None:
    ...
    if not cand:
        return
    # Ограничиваем скан за цикл, чтобы каждый проход был ~1-2 мин (иначе десятки
    # тысяч адресов сканируются минутами). Каждый цикл берём НОВЫЙ срез (ротация
    # смещения), чтобы со временем покрыть всех кандидатов.
    global _cand_offset
